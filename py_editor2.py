...
class Files:  # 定义文件基本参数
    ...
    def login(self, password, user=__author__.get('name')):
        self.user = user
...
def pop_win_message(m_title, message):
    mt = Td(args=(m_title, message))
    mt.target = pop_message(m_title, message, ),
    mt.start()


def translate(word):  # 翻译
    translator = Translator(to_lang='chinese')
    translation = translator.translate("{}".format(word))
    return translation
...
def ld():  # 朗读
    text = text_box.get(Files.BEGIN, Files.END)
    ld_threading = Td(target=speak(text), args=text_box.get(Files.BEGIN, Files.END))
    ld_threading.start()
...
def syntax_highlighting(master):  # 语法高亮
    global text_box
    percolator = Percolator(master)
...
def main():
    ...
    global runtf
    papers.init()
    papers.login(user='未知用户', password='20140620')
    runtf = True
    pack_bj()
    root.mainloop()
...
new_win_menu.add_command(label='新建窗口', command=new_win)
new_win_menu.add_command(label='新建带控制台的窗口', command=new_win)
...
editmenu.add_checkbutton(label="显示控制台", command=kzt_win, accelerator='Ctrl+W+K', variable=kzt_var)
editmenu.add_separator()
...
view_menu.add_command(label="查看状态栏", command=looks_ztl, accelerator='Ctrl+L')
...
# 语法高亮
syntax_highlighting(master=text_box)
# 定义窗口布局
...
root.protocol(Tool.QUIT, quit_window)
# 开启主循环
