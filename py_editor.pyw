...
def wow():
    ...
    # 设置程序缩放
    root.tk.call('tk', 'scaling', ScaleFactor / 95)
...
#         progressbarOne.after(10, loading)
#     else:
#         win.destroy()
...
def quit_window():  # 关闭窗口
    ...
    if quit_yesno is True:
        ...
        with open('reducing.inf', 'r+', encoding='UTF-8') as f:
            f.truncate()
            f.write(text_box.get(Files.BEGIN, Files.END))
        enter()
        root.quit()
...
def python_help():
    url_open('https://docs.python.org/zh-cn/3.11/')
...
def reducing():  # 还原
    ...
    if yn:
        with open('reducing.inf', 'r+', encoding=Files.encoding) as f:
            text = f.read()
...
def wi_hyphen():  # 设置字号
    ...
    sp = ttk.Spinbox(wi_hyphen_top, from_=5, to=40, command=oh, state='readonly')
    sp.set(hyphen)
    sp.pack()
...
def oiii():
    system('parse_peins.py')


def oi():
    my_td = Td(target=oiii)
    my_td.start()


def oc():
    system(
        r'explorer .\software_library_is_installed')
...
xhx = [u'F̲', u'E̲', u'O̲', u'V̲', u'W̲', u'I̲', u'H̲', u'C̲']
papers = Files()  # 实例化文件
...
statusbar = tk.Label(root, text="人生苦短,我用python", bd=1, relief=tk.SUNKEN, anchor=tk.W)
statusbar.pack(side=tk.BOTTOM, fill=tk.X)
ins_menu = tk.Menu(root, tearoff=False)
ins_menu.add_command(label='启动插件管理器', command=oi)
ins_menu.add_command(label='插件启动器', command=oc)
root_menu.add_cascade(label=f'插件({xhx[7]})', menu=ins_menu)
# 语法高亮
syntax_highlighting()
wow()
# # 魔法发生的地方
